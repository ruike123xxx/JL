...
RPA 发送回复 / 索要简历 / 发地址  ◀──JSON(answer+rpa_action)──  本项目
```
...
- **`rpa_action` 是机器可读枚举**：合法值定义在 [app/schemas.py](app/schemas.py) 的 `RPA_ACTIONS`（`reply_message` / `request_resume` / `send_company_address`）。新增动作必须同时改这里和 mock/真实 prompt。
