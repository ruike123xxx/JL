...
把"调哪个模型"与业务逻辑解耦。pipeline 只调抽象接口，不知道背后是 mock 还是通义。
**换模型 = 改 `.env` 的 `LLM_PROVIDER` + 加一个 provider 文件 + 在 base.py 注册。**
...
- `get_provider() -> LLMProvider` —— 按 `settings.llm_provider` 选实例：`mock` → `MockProvider`，`tongyi` → `TongyiProvider`，未知值抛错。
