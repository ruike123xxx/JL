...
定义单独的简历评价路由，由 [app/main.py](../main.py) 挂载。

- **`POST /resume/evaluate`** —— 图片/视频简历评分接口。
  - 入参：`ResumeEvaluationRequest`（candidate_id / resume_text、resume_image_url 或 resume_video_url / job_requirement）
  - 出参：`ResumeEvaluationResponse`（score / passed / threshold / basis / matched / risks）
  - 只评价简历与岗位匹配度，不生成聊天回复，不影响 `/reply` 的影刀响应结构
