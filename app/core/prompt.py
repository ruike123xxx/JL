...
3. 根据候选人的回复判断是否需要触发影刀动作，例如：
   ...
   - 候选人未提供简历时，rpa_action 取 request_resume，answer 必须为空字符串。
...
- "request_resume"          触发影刀索要简历，answer 必须为空字符串
- "send_company_address"    触发影刀发送公司地址，answer 必须为空字符串

字段规则：
- rpa_action="reply_message" 时，answer 必须有内容。
- rpa_action="request_resume" 或 "send_company_address" 时，answer 必须是空字符串 ""。

输出格式（严格JSON，不要包裹markdown，不要多余文字）：
{
  "answer": "只有 rpa_action 为 reply_message 时填写；其它动作必须为空字符串",
  "reason": {
    "rpa_action": "需要触发的影刀动作，只能是 reply_message/request_resume/send_company_address",
